...
def load_images():
    ...
    return images
# Funkcija za iscrtavanje šahovske table i figura
def draw(screen, board, selected_square, pieces):
    square_size = 800 // 8
    for row in range(8):
        for col in range(8):
            rect = pygame.Rect(BORDER_WIDTH + row * square_size, BORDER_WIDTH + col * square_size, square_size, square_size)
            pygame.draw.rect(screen, WHITE if (row + col) % 2 == 0 else BLACK, rect)

            piece = board.piece_at((7 - col) * 8 + row)
            if piece:
                piece_image = pieces[get_piece_name(piece)]
                screen.blit(piece_image, rect.topleft)

    if selected_square is not None:
        rect = pygame.Rect(BORDER_WIDTH + selected_square[0] * square_size, BORDER_WIDTH + (7 - selected_square[1]) * square_size, square_size, square_size)
...
# Funkcija za pretvaranje koordinata miša u koordinate šahovske table
def get_square(pos):
    ...
    row =(adjusted_pos[0] // square_size)
    col =(adjusted_pos[1] // square_size)
...
def save_game_to_pgn(board, engine, game_result):
    ...
    new_game.headers["White"] = "Human"
    new_game.headers["Black"] = "{}".format(engine.id["name"])
...
def main():
    ...
    clock = pygame.time.Clock()  # Dodajte ovu liniju
            ...
            if event.type == pygame.QUIT:
                ...
                if selected_square is not None and target_square is not None:
                    ...
                    if move in board.legal_moves:
                        display_move(move, board)  
                        board.push(move)

                selected_square = None

        draw(screen, board, selected_square, pieces)  
